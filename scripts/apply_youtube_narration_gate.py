...
def _apply_router_patch() -> bool:
    if not YOUTUBE_ROUTER.is_file():
        raise SystemExit("youtube narration core: youtube router missing")
    source = YOUTUBE_ROUTER.read_text(encoding="utf-8")
    if APPROVED_AUDIO_MARKER_CORE in source and APPROVED_AUDIO_DURABLE_MARKER in source:
        return False
    ...
    if source.count(RENDER_CALL_MARKER) != 1:
        raise SystemExit(
            f"youtube narration core: expected one canonical render call marker, found {source.count(RENDER_CALL_MARKER)}"
        )
...
def apply() -> bool:
    ...
    if not JS.is_file():
        raise SystemExit("youtube narration core: JS asset missing")
    ...
    if OLD_BUTTON_LOOKUP in source:
        ...
        raise SystemExit("youtube narration core: canonical video button lookup not found")
...
def check() -> None:
    ...
    if missing:
        raise SystemExit(f"youtube narration core: JS contract missing {missing}")

    router_source = YOUTUBE_ROUTER.read_text(encoding="utf-8")
    router_required = [
        APPROVED_AUDIO_MARKER_CORE,
        APPROVED_AUDIO_DURABLE_MARKER,
        'Path(_APPROVED_AUDIO_ROOT).resolve() / "youtube_narration_core_v1"',
        'approved_source == "youtube_narration_core_v1_approved"',
        'approved_meta.get("approved") is True',
        'approved_story_hash == approved_text_hash',
        '_record_production_artifact(',
        'source="approved_narration_core_v1"',
        'script["seed_audio_path"] = str(durable_audio_path)',
        'script["approved_narration_required"] = True',
        'script["approved_narration_audio_sha256"] = approved_audio_sha256',
    ...
    if router_missing:
        raise SystemExit(f"youtube narration core: approved-audio render contract missing {router_missing}")
    if APPROVED_AUDIO_MARKER_V1 in router_source or APPROVED_AUDIO_MARKER_V2 in router_source:
        raise SystemExit("youtube narration core: legacy approved-audio block still present")
...
def main() -> None:
    ...
    if args.apply:
        changed = apply()
        print("youtube narration core:", "applied" if changed else "already applied")
    if args.check:
        check()
