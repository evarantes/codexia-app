...
VIDEO_SETUP_NEW = '''            selected_image_paths = []\n            selected_primary_path = None\n            scene_image_pool = []\n            scene_image_seen = set()\n            scene_reuse_counts = {}\n            # CODEXIA_PARTIAL_IMAGE_RECOVERY_V1\n            # RecoveryImageCallBudget(plan), criado no início do render, é a fonte\n            # de verdade para saber se esta execução é uma recuperação parcial.\n            # Aqui apenas marcamos o ponto de integração sem sobrescrever esse\n            # estado já validado pelo teto rígido de chamadas pagas.\n            selected_raw = plan.get("selected_images") or plan.get("images") or []'''

VIDEO_GROUP_OLD = '''                selected_image_count=len(selected_image_paths),'''
VIDEO_GROUP_NEW = '''                # Numa recuperação parcial, as imagens selecionadas são apenas\n                # o conjunto preservado, não a meta final. O orçamento assinado\n                # acima define quantos grupos visuais devem existir.\n                selected_image_count=(0 if partial_image_recovery else len(selected_image_paths)),'''
