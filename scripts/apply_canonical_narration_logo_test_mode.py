...
def apply() -> bool:
    ...
    if LOGO_TAG not in index:
        ...
        INDEX.write_text(index.replace(BODY, f"    {LOGO_TAG}\n{BODY}", 1), encoding="utf-8")
        changed = True

    gate = GATE_JS.read_text(encoding="utf-8")
    gate_changed = False
    if MARKER not in gate:
        gate = _replace_once(gate, OLD_DIRECT, NEW_DIRECT, "listener direto legado")
        gate_changed = True
    if SOURCE_TEXT_MARKER not in gate:
        gate = _replace_once(gate, OLD_APPROVED_SAVE, NEW_APPROVED_SAVE, "saveApproved")
        gate = _replace_once(gate, OLD_CONTINUE_GUARD, NEW_CONTINUE_GUARD, "continue guard")
        gate = _replace_once(gate, OLD_INPUT_GUARD, NEW_INPUT_GUARD, "input guard")
        if GLOBAL_NETWORK_MARKER in gate:
            gate = _replace_once(gate, OLD_GLOBAL_FETCH_GUARD, NEW_GLOBAL_FETCH_GUARD, "global fetch guard")
        else:
            gate = _replace_once(gate, OLD_FETCH_GUARD, NEW_FETCH_GUARD, "fetch guard")
        gate_changed = True
    if gate_changed:
    ...
    if STRUCTURAL_RESOLVER_MARKER not in router and REQUEST_FALLBACK_MARKER not in router:
        router = _replace_once(router, OLD_BACKEND_APPROVED_IDS, NEW_BACKEND_APPROVED_IDS, "backend approved metadata fallback")
...
def check() -> None:
    ...
    if STRUCTURAL_RESOLVER_MARKER in router:
        required_router = [
    ...
    if missing_router:
        raise SystemExit(f"canonical narration/logo mode: contrato autenticado do backend incompleto: {missing_router}")
