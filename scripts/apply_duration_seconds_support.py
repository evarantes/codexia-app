#!/usr/bin/env python3
"""Adiciona duração em segundos ao fluxo de vídeos narrados.

Roda depois de ``apply_duration_confirmation_hardening.py``. Mantém o contrato
legado em minutos para compatibilidade, mas permite valores fracionários de
minuto (ex.: 15 s = 0,25 min) de ponta a ponta. Assim o modo de teste rápido
reduz de verdade roteiro, TTS e render, em vez de apenas alterar o rótulo da UI.

O patch é determinístico e idempotente.
"""
...
def _replace_all(text: str, old: str, new: str, *, expected: int, label: str) -> str:
    if text.count(new) >= expected:
        return text
    count = text.count(old)
    if count != expected:
        raise PatchError(f"{label}: esperado {expected} trecho(s) original(is), encontrado {count}")
...
def patch_index(text: str) -> str:
    text = _replace_once(
        ...
        label="index/duration-unit-controls",
    )
    ...
    text = _replace_once(
        ...
        '''                    ytStoryImproveInstruction: '',
                    ...
                    ytStoryDurationMin: 10,
                    ytStoryDurationMax: 15,''',
        label="index/duration-unit-state",
    )
    ...
    text = _replace_once(
        ...
        '''                    const durationRaw = Number(source.duration || 5);
                    const duration = Number.isFinite(durationRaw)
                        ...
                        : 5;''',
        label="index/preserve-subminute-duration",
    )
    ...
    text = _replace_once(
        ...
        '''                    if (!narrationText) {
                        ...
                        const min = Number(this.ytStoryDurationMin || 0);
                        const max = Number(this.ytStoryDurationMax || 0);
                        const fallbackValue = ((min || 0) + (max || min || (unit === 'seconds' ? 15 : 1))) / (max ? 2 : 1);
                        return unit === 'seconds'
                            ? Math.max(5, Math.round(fallbackValue || 15))
                            : Math.max(60, Math.round((fallbackValue || 1) * 60));
                    }''',
        label="index/prediction-fallback-supports-seconds",
    )

    duration_vars_old = '''                        const durationMin = Number(this.ytStoryDurationMin || 10);
                        const durationMax = Number(this.ytStoryDurationMax || durationMin);'''
    duration_vars_new = '''                        const durationUnit = this.ytStoryDurationUnit === 'seconds' ? 'seconds' : 'minutes';
    ...
    text = _replace_all(
        ...
        duration_vars_new,
        expected=2,
        label="index/story-text-duration-conversion",
    )
    ...
    text = _replace_all(
        ...
        expected=2,
        label="index/story-text-sends-duration-unit",
    )
    ...
    text = _replace_once(
        ...
        '''                        const durationUnit = this.ytStoryDurationUnit === 'seconds' ? 'seconds' : 'minutes';
                        ...
                        if (predictedSeconds > 0 && (predictedSeconds < requestedMinSeconds || predictedSeconds > requestedMaxSeconds)) {
                            ...
                            const deltaLabel = this.formatNarrationDurationHuman(deltaSeconds);
                            const directionLabel = isOver ? 'acima' : 'abaixo';''',
        label="index/video-submit-supports-seconds",
    )
    ...
    text = _replace_once(
        ...
        '''                            duration: duration,
                            ...
                            duration_override_approved: durationOverrideApproved,
                            auto_upload: !!this.ytStoryAutoUpload,''',
        label="index/video-payload-duration-unit",
    )
    ...
    text = _replace_once(
        ...
        label="index/draft-load-duration-unit",
    )
    ...
    text = _replace_once(
        ...
        '''                        const draftDurationUnit = this.ytStoryDurationUnit === 'seconds' ? 'seconds' : 'minutes';
                        ...
                        const meta = {
                            ...
                            instruction: String(this.ytStoryInstruction || '').trim(),''',
        label="index/draft-save-duration-unit",
    )
...
def patch_youtube_router(text: str) -> str:
    ...
    duration_override_approved: bool = False''',
        label="youtube/video-request-fractional-minutes",
    )
    ...
    text = _replace_once(
        ...
        "duration_override_approved": bool(payload.get("duration_override_approved")),''',
        label="youtube/dedupe-preserves-subminute-duration",
    )
    ...
    text = _replace_once(
        ...
        duration_override_approved = bool(getattr(request, "duration_override_approved", False))''',
        label="youtube/runtime-preserves-subminute-duration",
    )
    ...
    text = _replace_once(
        ...
        '''            script["duration_min"] = float(requested_min_minutes)
            ...
            script["duration_override_approved"] = duration_override_approved''',
        label="youtube/plan-keeps-exact-seconds",
    )
...
class StoryTextImproveRequest(BaseModel):
    ...
    duration_max: Optional[float] = None
    duration_unit: str = "minutes"''',
        label="youtube/story-text-accepts-fractional-minutes",
    )
    ...
    text = _replace_once(
        ...
        label="youtube/story-plan-duration-float",
    )
    text = _replace_once(
        text,
        ...
        label="youtube/story-plan-subminute-word-target",
    )
    text = text.replace(
...
def patch_story_review_editor(text: str) -> str:
    ...
    duration_max_minutes: Optional[float] = None,''',
        label="story-editor/fractional-duration-signature",
    )
    text = _replace_once(
        text,
    ...
    max_words = max(min_words + 5, int(max_m * 165))''',
        label="story-editor/subminute-word-range",
    )
    text = _replace_once(
        text,
        'DURAÇÃO: {min_m} a {max_m} minuto(s)',
        "DURAÇÃO: {round(min_m * 60)} a {round(max_m * 60)} segundos" + "' if max_m < 1 else f'" + "DURAÇÃO: {min_m:g} a {max_m:g} minuto(s)",
        label="story-editor/duration-prompt-human-unit",
    )
    # A substituição acima precisa resultar numa expressão f-string válida dentro
    # do template multilinha. Convertemos o trecho para uma expressão inline.
    text = text.replace(
        "DURAÇÃO: {round(min_m * 60)} a {round(max_m * 60)} segundos' if max_m < 1 else f'DURAÇÃO: {min_m:g} a {max_m:g} minuto(s)",
...
def patch_ai_generator(text: str) -> str:
    ...
    text = _replace_all(
        ...
        new,
        expected=2,
        label="ai-generator/subminute-story-word-target",
    )
    if MARKER not in text:
        text = text.rstrip() + f"\n\n# {MARKER}\n"
    return text
...
def apply(*, write: bool) -> int:
    ...
    for rel_path, patcher in PATCHERS.items():
        ...
        compile_check = rel_path.endswith('.py') and not rel_path.startswith('scripts/')
        if compile_check:
