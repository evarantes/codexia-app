...
# literal. Recovery remains confirmation-gated; logo-only and long-form jobs
# may use the same local renderer automatically. Long-form MoviePy composition
# is prohibitively expensive because every caption/motion layer is calculated
# in Python for thousands of frames; FFmpeg performs the equivalent timeline,
# real-audio captions and local music mix in one bounded native process.
VIDEO_FAST_BLOCK = VIDEO_FAST_BLOCK.replace(
    ...
    """            # Recovery requires the confirmed optimization hash. Logo-only
            ...
            try:
                _long_form_threshold = float(
                    (os.getenv("VIDEO_LONG_FORM_FFMPEG_MIN_SECONDS") or "180").strip() or "180"
                )
            except Exception:
                _long_form_threshold = 180.0
            _long_form_threshold = max(120.0, min(900.0, _long_form_threshold))
            _long_form_fast_mode = bool(
                float(target_video_duration or 0.0) >= _long_form_threshold
                and len(selected_image_paths or []) >= 2
                and bool(full_caption_timeline)
                and str(os.getenv("VIDEO_LONG_FORM_FFMPEG_RENDER") or "true").strip().lower()
                not in {"0", "false", "no", "off"}
            )
            _lightweight_confirmed = bool(
                _recovery_lightweight_confirmed
                or _logo_only_fast_mode
                or _long_form_fast_mode
            )
            if _lightweight_confirmed:""",
)
VIDEO_FAST_BLOCK = VIDEO_FAST_BLOCK.replace(
    ...
    '''                    progress_callback(
                        ...
                        if _logo_only_fast_mode
                            ...
                            "6/8 Preparando render FFmpeg para vídeo longo..."
                            if _long_form_fast_mode
...
VIDEO_FAST_BLOCK = VIDEO_FAST_BLOCK.replace(
    '''                render_report["final_video_duration_sec"] = round(_obtained_light_duration, 2)
                render_report.setdefault("duration_plan", {})''',
    '''                render_report["final_video_duration_sec"] = round(_obtained_light_duration, 2)
                # A validação pré-revisão exige métricas físicas de variedade.
                # No render FFmpeg cada caminho local representa um grupo visual
                # único; a timeline pode reutilizar o grupo apenas dentro da sua
                # própria janela narrativa, sem duplicar uma compra de imagem.
                _fast_unique_images = list(dict.fromkeys(
                    os.path.abspath(str(item or "").strip())
                    for item in (selected_image_paths or [])
                    if str(item or "").strip()
                ))
                _fast_visual_count = len(_fast_unique_images)
                render_report.setdefault("visual_plan", {})
                render_report["visual_plan"].update({
                    "requested_image_count": _fast_visual_count,
                    "group_count": _fast_visual_count,
                    "generated_image_count": _fast_visual_count,
                    "reused_image_count": 0,
                    "average_image_duration_sec": round(
                        float(target_video_duration or 0.0) / max(1, _fast_visual_count),
                        2,
                    ),
                    "renderer": str(_light_result.get("renderer") or "ffmpeg_concat_subtitles_v1"),
                })
                render_report["scene_visuals"] = [
                    {
                        "scene_number": _fast_index + 1,
                        "image_group_id": _fast_index + 1,
                        "image_path": _fast_path,
                        "source": "generated_or_selected_local_asset",
                        "reused": False,
                    }
                    for _fast_index, _fast_path in enumerate(_fast_unique_images)
                ]
                render_report.setdefault("duration_plan", {})''',
)
VIDEO_FAST_BLOCK = VIDEO_FAST_BLOCK.replace(
    '''                    "captions_synced_with_audio": bool(full_caption_timeline),
                    "uses_official_scene_timeline": True,''',
    '''                    "captions_synced_with_audio": bool(full_caption_timeline),
                    "timeline_source": str(caption_timeline_source or "unknown"),
                    "uses_official_scene_timeline": True,''',
)
VIDEO_FAST_BLOCK = VIDEO_FAST_BLOCK.replace(
    '''                _light_result = render_lightweight_recovery_video(
                    output_path=_recovery_output_path,''',
    '''                # Persiste o inventário antes de iniciar o subprocesso. Se o
                # executor cair, a tela e o retry ainda sabem exatamente quais
                # ativos estão completos e podem ser reaproveitados.
                _checkpoint_task_id = str(getattr(self, "_codexia_task_id", "") or "").strip()
                if _checkpoint_task_id:
                    try:
                        from app.services.task_manager import merge_task_result as _merge_checkpoint_result
                        _checkpoint_image_paths = list(dict.fromkeys(
                            os.path.abspath(str(item or "").strip())
                            for item in (selected_image_paths or [])
                            if str(item or "").strip()
                        ))
                        _checkpoint_expected_images = int(
                            (visual_group_plan or {}).get("target_image_count")
                            or len(_checkpoint_image_paths)
                        )
                        _checkpoint_caption_end = max(
                            [float(item.get("end") or 0.0) for item in (full_caption_timeline or [])]
                            or [0.0]
                        )
                        _checkpoint_sync_delta = abs(
                            float(_checkpoint_caption_end or 0.0)
                            - float(actual_total_audio_dur or 0.0)
                        )
                        _checkpoint_sync_tolerance = max(
                            1.0,
                            float(actual_total_audio_dur or 0.0) * 0.02,
                        )
                        _checkpoint_text_match = bool(
                            normalized_caption_text
                            and normalized_tts_text
                            and normalized_caption_text == normalized_tts_text
                        )
                        _checkpoint_timing_verified = bool(
                            caption_timeline_source in REAL_AUDIO_CAPTION_TIMELINE_SOURCES
                        )
                        _checkpoint_director_compatible = bool(
                            full_caption_timeline
                            and _checkpoint_text_match
                            and _checkpoint_timing_verified
                            and _checkpoint_sync_delta <= _checkpoint_sync_tolerance
                        )
                        _merge_checkpoint_result(_checkpoint_task_id, {
                            "artifact_checklist": {
                                "version": 1,
                                "target_duration_sec": round(float(target_video_duration or 0.0), 3),
                                "script": {
                                    "complete": True,
                                    "preserved": True,
                                },
                                "images": {
                                    "actual": len(_checkpoint_image_paths),
                                    "expected": _checkpoint_expected_images,
                                    "complete": bool(
                                        _checkpoint_expected_images > 0
                                        and len(_checkpoint_image_paths) >= _checkpoint_expected_images
                                    ),
                                    "preserved": True,
                                },
                                "narration": {
                                    "duration_sec": round(float(actual_total_audio_dur or 0.0), 3),
                                    "target_sec": round(float(target_video_duration or 0.0), 3),
                                    "complete": True,
                                    "preserved": True,
                                },
                                "captions": {
                                    "duration_sec": round(_checkpoint_caption_end, 3),
                                    "target_sec": round(float(actual_total_audio_dur or 0.0), 3),
                                    "entries": len(full_caption_timeline or []),
                                    "source": str(caption_timeline_source or "unknown"),
                                    "complete": bool(full_caption_timeline),
                                    "preserved": True,
                                },
                                "narration_caption_compatibility": {
                                    "compatible": _checkpoint_director_compatible,
                                    "text_matches": _checkpoint_text_match,
                                    "timing_source_verified": _checkpoint_timing_verified,
                                    "timing_source": str(caption_timeline_source or "unknown"),
                                    "duration_difference_sec": round(_checkpoint_sync_delta, 3),
                                    "tolerance_sec": round(_checkpoint_sync_tolerance, 3),
                                    "director_verdict": (
                                        "Narração e legenda compatíveis"
                                        if _checkpoint_director_compatible
                            ...
                            "caption_checkpoint": {
                                ...
                                "duration_sec": round(_checkpoint_caption_end, 3),
                                "entries": len(full_caption_timeline or []),
                                "timeline": list(full_caption_timeline or []),
                            },
                        })
                    except Exception as _checkpoint_error:
                        print(f"Aviso: não foi possível persistir inventário pré-render: {_checkpoint_error}")
...
def check() -> None:
    ...
    required_video = (
        ...
        'VIDEO_LONG_FORM_FFMPEG_MIN_SECONDS',
        'VIDEO_LONG_FORM_FFMPEG_RENDER',
        '"long_form_bounded_ffmpeg"',
        '"ffmpeg_long_form_v1"',
        '"average_image_duration_sec": round(',
        '"timeline_source": str(caption_timeline_source or "unknown")',
