...
HELPER_BLOCK = r'''def _intelligent_retry_visual_materials(task_id: str, payload_override: Optional[Dict[str, Any]] = None) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        ...
        settings_row = None
        try:
            settings_row = db.query(Settings).order_by(Settings.id.desc()).first()
        except Exception:
            settings_row = None
        if unit_cost <= 0 and settings_row is not None:
            try:
                unit_cost = max(0.0, float(getattr(settings_row, "image_cost_unit", 0.0) or 0.0))
            except Exception:
                unit_cost = 0.0
        if unit_cost <= 0:
            try:
                ...
                cost_mode = str(
                    payload.get("cost_mode")
                    or payload.get("quality_mode")
                    or payload.get("generation_mode")
                    or "balanced"
                ).strip().lower()
                unit_cost = max(
                    0.0,
                    float(image_profile_for_mode(cost_mode).get("image_unit_cost_usd") or 0.0),
                )
            except Exception:
                unit_cost = 0.05

        try:
            audio_unit_cost = float(
                payload.get("voice_cost_unit")
                or (getattr(settings_row, "voice_cost_unit", 0.0) if settings_row is not None else 0.0)
                or os.getenv("YOUTUBE_AUTO_TTS_MINUTE_COST_UNIT")
                or 0.0120
            )
        except Exception:
            audio_unit_cost = 0.0120
        audio_unit_cost = max(0.0, audio_unit_cost)
        try:
        ...
        current_failure_message = str(getattr(row, "message", "") or "").strip().lower()
        image_only_quality_failure = bool(
            "image_count_minimum" in current_failure_message
            and audio_path
        )
        if strict_quality:
            try:
            ...
            plan = build_visual_quality_completion_plan(
                ...
                audio_unit_cost_usd_per_minute=audio_unit_cost,
                usd_brl=usd_brl,
                regenerate_narration=not image_only_quality_failure,
            )
        else:
